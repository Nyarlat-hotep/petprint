...
*)
*/
function BO(e,t=128){let{data:n,width:r,height:i}=e,a=new Uint8Array(r*i);for(let e=0,r=3;e<a.length;e++,r+=4)a[e]=+(n[r]>t);return a}function VO(e,t,n){let r=t,i=n,a=-1,o=-1;for(let s=0;s<n;s++)for(let n=0;n<t;n++)e[s*t+n]&&(n<r&&(r=n),s<i&&(i=s),n>a&&(a=n),s>o&&(o=s));return a<0?null:{x:r,y:i,w:a-r+1,h:o-i+1}}function HO(e,t,n){let r=new Uint8Array(n.w*n.h);for(let i=0;i<n.h;i++)for(let a=0;a<n.w;a++)r[i*n.w+a]=e[(i+n.y)*t+(a+n.x)];return r}let UO={model:`isnet_fp16`,device:`gpu`,output:{format:`image/png`,quality:1}},WO=(e,t,n)=>self.postMessage({type:`progress`,key:e,current:t,total:n}),GO=Promise.resolve();self.onmessage=e=>{let{type:t,blob:n,id:r}=e.data;t===`preload`?GO=GO.then(()=>RO({...UO,progress:WO}).catch(()=>{})):t===`run`&&(GO=GO.then(()=>KO(r,n)))};async function KO(e,t){try{let n=await zO(t,{...UO,progress:WO}),r=await createImageBitmap(n),i=new OffscreenCanvas(r.width,r.height),a=i.getContext(`2d`);a.drawImage(r,0,0);let o=BO(a.getImageData(0,0,r.width,r.height),64),s=VO(o,r.width,r.height);if(!s)throw Error(`No silhouette detected. Try a clearer photo.`);let c=HO(o,r.width,s),l=new OffscreenCanvas(s.w,s.h);l.getContext(`2d`).drawImage(i,s.x,s.y,s.w,s.h,0,0,s.w,s.h);let u=await l.convertToBlob({type:`image/png`});self.postMessage({type:`done`,id:e,result:{mask:c,width:s.w,height:s.h,previewBlob:u,bbox:s,imageWidth:r.width,imageHeight:r.height}},[c.buffer])}catch(t){self.postMessage({type:`error`,id:e,message:t?.message||String(t)})}}})();